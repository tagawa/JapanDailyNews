...
def generate_audio(chat_content, voice_id, elevenlabs_api_key, today):
    try:
        ...
        if audio_output.status_code == 200:
            output_file_name = f"japandailynews_{today}.mp3"
...
def create_text_files(chat_content, today, output_file_name):
    try:
        ...
        transcript_file_name = f"japandailynews_{today}.txt"
    ...
    try:
        ...
        # Get the current date & time (in the Japan timezone)
        japan_timezone = ZoneInfo("Asia/Tokyo")
        current_datetime_japan = datetime.now(japan_timezone)
        formatted_datetime = current_datetime_japan.strftime("%Y-%m-%d %H:%M:%S %z")
        
        # Format the date in a human-friendly format, e.g. April 5th, 2023
        today_human = current_datetime_japan.strftime("%B ") + ordinal(current_datetime_japan.day) + current_datetime_japan.strftime(", %Y")        
...
def read_config_from_env():
    load_dotenv()
    config = {
        'news_feed': "https://www3.nhk.or.jp/rss/news/cat1.xml",  # Japanese society news stories from NHK
        'openai_api_key': os.getenv("OPENAI_API_KEY"),
        'elevenlabs_api_key': os.getenv("ELEVENLABS_API_KEY"),
        'ia_access_key': os.getenv("IA_ACCESS_KEY"),
        'ia_secret_key': os.getenv("IA_SECRET_KEY"),
        'github_token': os.getenv("GITHUB_TOKEN"),
        'github_repo': "tagawa/japandailynews"  # Replace with your GitHub username and repo name
    }
    return config
...
def process_audio_and_metadata(chat_content, elevenlabs_api_key, today, voice_id="21m00Tcm4TlvDq8ikWAM"):
    ...
    title = f"Japan Daily News - {today}"
    artist = "Japan Daily News"
    album = "Japan Daily News Podcast"
    ...
    cover_image_path = "japandailynews_cover-image_600.png"
...
def upload_to_archive(config, output_file_name, today):
    ...
    identifier = f"japandailynews"
    metadata = {
        "title": f"Japan Daily News - {today}",
        "creator": "Japan Daily News",
        "date": today,
        "language": "eng",
        "licenseurl": "https://creativecommons.org/licenses/by-nc-sa/4.0/",
        "mediatype": "audio",
        "collection": "opensource_audio",  # Change this to your desired collection
        "subject": "podcast; japan; daily; news"
    }
